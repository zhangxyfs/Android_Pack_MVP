...
//            EToast.get().showToast(context, "没有可以打开文件的软件！");
    ...
    public static boolean isHasWPS() {
        ...
        try {
            PackageInfo packageInfo = Appli.Companion.getContext().getPackageManager().getPackageInfo(WpsModel.PackageName.NORMAL, 0);
            isHasWps = packageInfo != null;
        } catch (PackageManager.NameNotFoundException e) {
            Utils.log(WPSUtils.class.getName(), "no wps");
        }
        return isHasWps;
    }
}
