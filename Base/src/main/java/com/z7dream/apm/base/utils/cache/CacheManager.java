...
import com.z7dream.apm.base.utils.explorer.MagicExplorer;
...
public class CacheManager {
    ...
    public static String getSystemPicCachePath() {
        File file = Appli.Companion.getContext().getExternalFilesDir(Environment.DIRECTORY_PICTURES);
        if (file != null) {
    ...
    public static String getRelativePath(int which) {
        return getSavePath(getRelativePath(), which);
    }
    ...
    public static String getPath(int which, String childFolderName) {
        String path = getCachePath(Appli.Companion.getContext(), which);
    ...
    public static String getIMPath(int which) {
        String rootPath = getCachePath(Appli.Companion.getContext(), IM);
        if (rootPath.endsWith(File.separator)) {
            rootPath = rootPath.substring(0, rootPath.length() - 1);
        }
        rootPath = getSavePath(rootPath, which);
    ...
    public static String getDownloadFile(String downloadUrl, int which) {
        ...
        if (fileName.split("\\.").length > 1) {
            String filePath = getCachePath(Appli.Companion.getContext(), which) + fileName;
    ...
    public static String getOffLineCachePath(int which) {
        String rootPath = getCachePath(Appli.Companion.getContext(), OFF_LINE_CACHE);
        if (rootPath.endsWith(File.separator)) {
            rootPath = rootPath.substring(0, rootPath.length() - 1);
        }
//        rootPath = getSavePath(rootPath, which);
